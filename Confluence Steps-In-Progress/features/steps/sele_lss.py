...
def check_body(context):
    ...
    return bool(context.driver.find_elements_by_id('lss-content')) and (body_heading == 'Trusted inspectors. Honest evaluation. Timely service.') and (schedule_button == 'Schedule Appointment') and login_button == 'or Log in'
...
def only_vin_error_message(context):
    return context.driver.find_elements_by_class_name('error-message')[0].text == 'Enter a valid VIN'

def only_ac_no_error_message(context):
    return context.driver.find_elements_by_class_name('error-message')[0].text == 'Enter a valid account number'

def both_vin_and_ac_no_error_message(context):
    return context.driver.find_elements_by_class_name('error-message')[0].text == 'Enter a valid VIN' and context.driver.find_elements_by_class_name('error-message')[1].text == 'Enter a valid account number'
...
def valid_vin_invalid_ac_no_error_message(context):
    ...
    return context.driver.find_element_by_class_name('error-message').text == 'We were unable to match the VIN and account number you entered. Please check your entries and try again.'
...
def get_about_page_url(context):
    ...
    return context.driver.find_element_by_link_text('About AiM').get_attribute('href')

def click_about_page_navbar_item(context):
    context.driver.get(get_about_page_url(context))

    

def about_page_navbar_displayed(context):
    menu_items= context.driver.find_element_by_id('shiftnav-toggle-main').text.split('\n')
    main_menu_text_is_shown = menu_items[0] == 'Main Menu'
    about_text_is_shown = menu_items[1] == 'Accounts'
    about_page_aim_logo_is_shown = bool(context.driver.find_element_by_id('aim_logo'))
    about_page_title_is_shown = context.driver.find_element_by_class_name("main-title").text == 'About Us'

    about_page_pic = context.driver.find_element_by_class_name("wp-image-2502").get_attribute('src').split('.')[-1]
    about_page_pic_is_shown = about_page_pic == 'png'

    about_page_body_is_shown = bool(context.driver.find_element_by_class_name('avia_textblock'))

    about_page_portfolio_box_is_shown = bool(context.driver.find_element_by_id('portfoliobox-2'))

    about_page_services = context.driver.find_elements_by_class_name('news-link')
    ...
    about_page_footer_is_displayed = bool(context.driver.find_element_by_id('socket'))
    about_page_text_is_correct = context.driver.find_element_by_id('socket').text == '© Copyright 2017 - Alliance Inspection Management'
...
def get_heading(context):
    return context.driver.find_elements_by_tag_name('h1')[0].text

def get_subheadings(context):
    return context.driver.find_elements_by_tag_name('h2')

def check_text(context, tag, text):
    return context.driver.find_elements_by_tag_name(tag)[0].text == text

def check_number_of_paras(context, num):
    return len(context.driver.find_elements_by_tag_name('p')) == num
...
def check_privacy_policy_page_subheadings(context):
    ...
    #
    no_of_paras_correct = len(context.driver.find_elements_by_tag_name('p')) == 36
...
def check_terms_page_body(context):
    h2 = get_subheadings(context)
    sz_correct = len(h2) == 1 
    no_of_paras_correct = len(context.driver.find_elements_by_tag_name('p')) == 17 
...
def check_contact_page_body(context):
    h2 = get_subheadings(context)
    sz_correct = len(h2) == 1 
    no_of_paras_correct = len(context.driver.find_elements_by_tag_name('h4')) == 1 
    ...
    contact_page_pic = context.driver.find_element_by_class_name("csr-icon").get_attribute('src').split('.')[-1]
...
def appointment_already_scheduled_page_displayed(context):
    currently_on_existing_appointment_page = context.driver.current_url == 'https://selfschedule-qa.aiminspect.com/existing-appointment'
    existing_appointment_page_logo_shown = context.driver.find_element_by_class_name('rocket-icon').get_attribute('src').split('.')[-1] == 'png'
    existing_appointment_page_heading_shown = context.driver.find_element_by_tag_name('h2').text == 'It looks like you already have an appointment scheduled.'
    confirmation_number_shown = bool(context.driver.find_element_by_id('confirmationNo').text)
    appointment_date_time_shown = bool(context.driver.find_element_by_id('dateTime').text)
    appointment_loc_shown = bool(context.driver.find_element_by_id('apptLocation').text)
    info_text_shown = context.driver.find_element_by_tag_name('h5').text == 'If you have any questions about your appointment, please contact our team.'
    help_contact_shown = bool(context.driver.find_elements_by_class_name('contact-container')[0].text)
    call_center_hours_shown = bool(context.driver.find_element_by_id('callCenterHours').text)
    #
...
def inspection_completed_page_displayed(context):
    currently_on_inspection_completed_page = context.driver.current_url == 'https://selfschedule-qa.aiminspect.com/condition'
    greeting_message_shown = ('Hello' in context.driver.find_element_by_class_name('col-xs-12').text) == True
    headbar_shown = bool(context.driver.find_element_by_class_name('header-bar'))
    headbar_text_correct = context.driver.find_element_by_class_name('header-bar').text == 'Overview'
    vin_shown = len(context.driver.find_elements_by_class_name('vin')) > 0
    inspection_date_and_status_shown = len(context.driver.find_elements_by_class_name('inspection-fact')) == 2
    view_cr_button_exists = bool(context.driver.find_element_by_partial_link_text('View CR').get_attribute('href'))
    get_nav_tabs = context.driver.find_elements_by_class_name('nav-tabs')
...
def standard_photos_tab_active_by_default(context):
    return context.driver.find_elements_by_class_name('active')[0].text == 'STANDARD PHOTOS'

def click_damaged_photos_tab(context):
    context.driver.find_element_by_partial_link_text('DAMAGE PHOTOS').click()

def damaged_photos_tab_active(context):
    return context.driver.find_elements_by_class_name('active')[0].text == 'DAMAGE PHOTOS'

def standard_photos_tab_inactive(context):
    return context.driver.find_elements_by_tag_name('li')[3].get_attribute('class') == ''

def damage_photos_tab_inactive(context):
    return context.driver.find_elements_by_tag_name('li')[4].get_attribute('class') == ''

def click_img(context):
    context.driver.find_elements_by_class_name('vehicle-photo')[1].click()

def image_shown(context):
    return context.driver.find_elements_by_class_name('vehicle-photo')[1].get_attribute('src').split('.')[-1] == 'jpg'

def view_cr_button_shown(context):
    return bool(context.driver.find_elements_by_partial_link_text('View CR')[0])

def click_view_cr_button(context):
    return context.driver.find_elements_by_partial_link_text('View CR')[0].click()

def get_cr_button_url(context):
    return context.driver.find_elements_by_partial_link_text('View CR')[0].get_attribute('href')
...
def verify_account_info_page_shown(context):
    verify_account_url_navigated = context.driver.current_url ==
...
def get_phone_field(context):
    return context.driver.find_element_by_id('phone')

def get_email_field(context):
    return context.driver.find_element_by_id('email')

def no_phone_error_message_shown(context):
    give_val_to_field(get_phone_field(context), "")
    context.driver.find_element_by_class_name('lss-form-input').click()
    give_val_to_field(get_phone_field(context), "")
    error_message_correct = context.driver.find_elements_by_class_name('error-message')[0].text == 'This field is required'
...
def invalid_phone_error_message_shown(context):
    ...
    error_message_correct = context.driver.find_elements_by_class_name('error-message')[0].text == 'Enter a valid phone number'
    return error_message_correct
...
def no_email_error_message_shown(context):
    ...
    context.driver.find_element_by_class_name('lss-form-input').click()
    give_val_to_field(get_email_field(context), "")
    error_message_correct = context.driver.find_elements_by_class_name('error-message')[0].text == 'Enter a valid email address'
    return error_message_correct
...
def submit_form_with_valid_email_and_phone(context):
    give_val_to_field(get_email_field(context), "")
    give_val_to_field(get_email_field(context), "abc@def.com") 
    give_val_to_field(get_phone_field(context), "")
    give_val_to_field(get_phone_field(context), "1234567891")
    context.driver.find_element_by_class_name('btn-primary').click()
    context.driver.implicitly_wait(30)



    #  https://selfschedule-qa.aiminspect.com/location