Feature: TO DO
    ...
    Given User is logged in   # https://crm-qa.aiminspect.com
    And current page is add organization    # https://crm-qa.aiminspect.com/#!/organization/add
    When 
    # selects to create a dealer/organization  
    # And 
    selects a customer from the drop down option that is click enabled (Nisan as of now)
    And chooses Dealer as the org type
    And enter a organization name 
    And use billing address and inspection location as same by checking off the check box for 'same as billing address'
    And input a Street 1 to address
    And selects state from drop down option that is click enabled 
    And input a valid zip code for state
    And choose Dealer code or NNA Dealer code from drop down menu 
    And input valid Organization code
    And click on 'Submit' button to create the organization
    Then AIM Assistant Manager page is displayed
...
# CHECK
    ...
    Scenario: Import dealer request and make sure it auto schedules.
        ...
        When dealer request is imported       # how to use api to import dealer request (Nissan Dealer Request Processing DocumentationRequest CreateDetailsURL/api/v2/inspection-request/import/)  
    ...
    Scenario: Using the app, UTC a dealer request where a message is sent to scheduler 
        ...
        Given dealer request is imported using postman          # how to import using postman
    ...
    And update the address from XYZ to ABC 
    And Select a date and click on 'check availability'
    And Then click on 'unable to schedule' when the button is displayed
    And Then type in a comment in 'unable to schedule' field and then save the changes
    And Then go to back to request and try to schedule the appointment and look at the address being displayed.
    Then the address that should be displayed is address ABC and not XYZ
